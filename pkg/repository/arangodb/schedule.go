...
func GetLessThanHalfSchedulesByProviderID(ctx context.Context, providerID string) ([]*model.Schedule, int) {
	result := []*model.Schedule{}

	query := `
		FOR d IN Schedules 
			FILTER d.providerId == @providerId 
				AND d.courseStartAt > @timeNow 
				AND d.courseStartAt <= @courseStartAt 
			RETURN d`
	bindVars := map[string]interface{}{
		"timeNow":       time.Now().Unix(),
		"courseStartAt": time.Now().Add(30 * time.Minute).Unix(),
		"providerId":    providerID,
	}
	cursor, err := db.Query(ctx, query, bindVars)
	if err != nil {
		logger.Errorf("[ArangoDB][GetSchedulesByCourseStartAt] query failed: %v", err)
	...
	for {
		...
		if driver.IsNoMoreDocuments(err) {
			...
			logger.Errorf("[ArangoDB][GetSchedulesByCourseStartAt] cursor failed: %v", err)
			return result, constant.ArangoDB_Driver_Failed
		}
...
func GetReadyStartSchedules(ctx context.Context) ([]*model.Schedule, int) {
	...
	bindVars := map[string]interface{}{
		"timeNow":       time.Now().Unix(),
		"courseStartAt": time.Now().Add(10 * time.Minute).Unix(),
	}
	cursor, err := db.Query(ctx, query, bindVars)
	if err != nil {
