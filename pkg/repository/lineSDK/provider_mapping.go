...
type provider_mapping struct {
	...
	dirty map[string]*entryProvider_mapping
...
type readOnlyProvider_mapping struct {
	m       map[string]*entryProvider_mapping
...
func (m *provider_mapping) Store(key string, value *model.Provider) {
	...
	if e, ok := read.m[key]; ok {
		...
		if !read.amended {
			...
			m.dirtyLocked()
			m.read.Store(readOnlyProvider_mapping{m: read.m, amended: true})
		}
		m.dirty[key] = newEntryProvider_mapping(value)
	}
	m.mu.Unlock()
}
...
func (e *entryProvider_mapping) unexpungeLocked() (wasExpunged bool) {
	return atomic.CompareAndSwapPointer(&e.p, expungedProvider_mapping, nil)
}
...
func (m *provider_mapping) Delete(key string) {
	m.LoadAndDelete(key)
}
...
func (m *provider_mapping) Range(f func(key string, value *model.Provider) bool) {
	...
	if read.amended {
		...
		read, _ = m.read.Load().(readOnlyProvider_mapping)
		if read.amended {
			read = readOnlyProvider_mapping{m: m.dirty}
			m.read.Store(read)
			m.dirty = nil
			m.misses = 0
		}
...
func (m *provider_mapping) missLocked() {
	m.misses++
	if m.misses < len(m.dirty) {
		return
	}
	m.read.Store(readOnlyProvider_mapping{m: m.dirty})
	m.dirty = nil
	m.misses = 0
}

func (m *provider_mapping) dirtyLocked() {
	if m.dirty != nil {
		return
	}

	read, _ := m.read.Load().(readOnlyProvider_mapping)
	m.dirty = make(map[string]*entryProvider_mapping, len(read.m))
	for k, e := range read.m {
...
func (e *entryProvider_mapping) tryExpungeLocked() (isExpunged bool) {
	p := atomic.LoadPointer(&e.p)
	for p == nil {
		if atomic.CompareAndSwapPointer(&e.p, nil, expungedProvider_mapping) {
			return true
		}
		p = atomic.LoadPointer(&e.p)
	}
	return p == expungedProvider_mapping
}
